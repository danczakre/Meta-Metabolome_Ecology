...
# Load in bNTI
char = read.csv("Shoreline_MCD_bNTI_999.csv", row.names = 1)
trans = read.csv("Shoreline_TD_bNTI_999.csv", row.names = 1)
weig = read.csv("Shoreline_TWCD_bNTI_999.csv", row.names = 1)

# Load in RCBC
char.rcbc = read.csv("Shoreline_MCD_RCBC_9999.csv", row.names = 1)
trans.rcbc = read.csv("Shoreline_TD_RCBC_9999.csv", row.names = 1)
weig.rcbc = read.csv("Shoreline_TWCD_RCBC_9999.csv", row.names = 1)
...
char.rcbc = char.rcbc[row.names(char), colnames(char), drop = F]
trans.rcbc = trans.rcbc[row.names(trans), colnames(trans), drop = F]
...
char.rcbc[abs(char) > 2] = NA
trans.rcbc[abs(trans) > 2] = NA
...
# Running a Mantel correlataion
mant = data.frame(Comparison = rep(NA, 3), Mantel_r = NA, p_value = NA)
mant[1,] = c("Characteristics -> Transformations", mantel(as.dist(char), as.dist(trans))$statistic, mantel(as.dist(char), as.dist(trans))$signif)
mant[2,] = c("Characteristics -> Weighted", mantel(as.dist(char), as.dist(weig))$statistic, mantel(as.dist(char), as.dist(weig))$signif)
mant[3,] = c("Weighted -> Transformations", mantel(as.dist(weig), as.dist(trans))$statistic, mantel(as.dist(weig), as.dist(trans))$signif)
...
char.rcbc = melt(as.matrix(char.rcbc)); char.rcbc$Tree = "Characteristics"
trans.rcbc = melt(as.matrix(trans.rcbc)); trans.rcbc$Tree = "Transformations"
weig.rcbc = melt(as.matrix(weig.rcbc)); weig.rcbc$Tree = "Weighted"
...
char.rcbc = char.rcbc[!is.na(char.rcbc$value),]
trans.rcbc = trans.rcbc[!is.na(trans.rcbc$value),]
weig.rcbc = weig.rcbc[!is.na(weig.rcbc$value),]

# Determining ecological processes fractionation
eco.proc = data.frame(hom.sel = rep(NA, 3), hom.dis = NA, und = NA, dis.lim = NA, var.sel = NA, 
                      row.names = c("Characteristics", "Transformation", "Weighted"))
eco.proc$hom.sel = c((length(which(char$value <= -2))/length(char$value)), (length(which(trans$value <= -2))/length(trans$value)), (length(which(weig$value <= -2))/length(weig$value)))
eco.proc$hom.dis = c((length(which(char.rcbc$value <= -0.95))/length(char$value)), 
                     (length(which(trans.rcbc$value <= -0.95))/length(trans$value)), 
                     (length(which(weig.rcbc$value <= -0.95))/length(weig$value)))
eco.proc$und = c((length(which(abs(char.rcbc$value) < 0.95))/length(char$value)), 
                 (length(which(abs(trans.rcbc$value) < 0.95))/length(trans$value)), 
                 (length(which(abs(weig.rcbc$value) < 0.95))/length(weig$value)))
eco.proc$dis.lim = c((length(which(char.rcbc$value >= 0.95))/length(char$value)), 
                     (length(which(trans.rcbc$value >= 0.95))/length(trans$value)), 
                     (length(which(weig.rcbc$value >= 0.95))/length(weig$value)))
eco.proc$var.sel = c((length(which(char$value >= 2))/length(char$value)), (length(which(trans$value >= 2))/length(trans$value)), (length(which(weig$value >= 2))/length(weig$value)))

eco.proc = melt(as.matrix(eco.proc))
eco.proc$value = (eco.proc$value*100)

ggplot(data = eco.proc, aes(x = Var2, y = value, fill = Var1))+
...
ggplot(data = eco.proc, aes(x = "", y = value, fill = Var2))+
  ...
  xlab(NULL) + ylab(NULL)+
  facet_grid(Var1~.)+
  scale_fill_manual(values = c("dodgerblue3", "green4", "gray70", "darkorchid3", "firebrick3"))+
  theme_bw()+
